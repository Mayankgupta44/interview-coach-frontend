...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-card/90 backdrop-blur shadow-sm">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-3 rounded-xl px-2 py-1.5 transition-all duration-200 hover:bg-appBg"
          >
            <Avatar
              name={user?.fullName}
          ...
          {open && (
            <div className="absolute right-0 mt-3 w-56 overflow-hidden rounded-xl bg-card shadow-soft">
              ...
              <div className="p-2">
                ...
                <DropdownLink to="/documents" label="Resume & JD" setOpen={setOpen} />
                <DropdownLink to="/interviews" label="Interviews" setOpen={setOpen} />
      ...
      <nav className="flex gap-1 overflow-x-auto px-4 py-2 lg:hidden">
        ...
        <NavLink to="/documents" className={navClass}>Resume</NavLink>
        <NavLink to="/skill-gap" className={navClass}>Skill Gap</NavLink>
        <NavLink to="/interviews" className={navClass}>Interviews</NavLink>
      </nav>
    </header>
  );
}
...
function DropdownLink({ to, label, setOpen }) {
  return (
    <Link
      ...
      className="block rounded-lg px-3 py-2 text-sm font-medium text-textSecondary transition-all duration-200 hover:bg-appBg hover:text-textPrimary"
    >
      {label}
    </Link>
  );
